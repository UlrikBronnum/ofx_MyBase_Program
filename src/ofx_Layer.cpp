...
void ofx_Layer::update(unsigned char* input_image){
    ...
    if(layer_type == "diffuse"){
        tex.loadData(Base_Image.image,image_w,image_h,GL_RGB);
        hist.update(Base_Image.image,image_w,image_h,true);
    }else if(layer_type == "height"){
        tex_grey.loadData(Base_Image.image,image_w,image_h,GL_LUMINANCE);
        hist.update(Base_Image.image,image_w,image_h,false);
    }else if(layer_type == "normal"){
        tex_grey.loadData(Base_Image.image,image_w,image_h,GL_LUMINANCE);
        hist.update(Base_Image.image,image_w,image_h,false);
    }
...
void ofx_Layer::update(){
    if(Canvas_Storage.size() > Layer_Buttons.Menu_State() && Layer_Buttons.Menu_State() != -1){
        ...
        {
            hist.update(Canvas_Storage[Layer_Buttons.Menu_State()]->image,image_w,image_h,true);
            image_grey = false;
            tex.loadData(Canvas_Storage[Layer_Buttons.Menu_State()]->image,image_w,image_h,GL_RGB);
        }
        else if(Canvas_Storage[Layer_Buttons.Menu_State()]->Image_Channel_Count() == 1)
        {
            hist.update(Canvas_Storage[Layer_Buttons.Menu_State()]->image,image_w,image_h,false);
            image_grey = true;
            tex_grey.loadData(Canvas_Storage[Layer_Buttons.Menu_State()]->image,image_w,image_h,GL_LUMINANCE);
        }
    }else{
        hist.update(Base_Image.image,image_w,image_h,true);
        image_grey = false;
...
void ofx_Layer::delete_Layer(int index ){
    ...
    cout << Layer_Buttons.Menu_State() << endl;
...
void ofx_Layer::draw(int x, int y , unsigned char color, int window_w, int window_h){
    ...
    Layer_Buttons.draw(window_w -button_w, window_h/3,color ,  window_w,  window_h);
    if(Layer_Buttons.Height() + window_h/3 > window_h){
        Layer_Com_Buttons.draw(window_w -button_w - button_h - 20, window_h/3,color ,  window_w,  window_h);
    }else{
        Layer_Com_Buttons.draw(window_w -button_w - button_h , window_h/3,color ,  window_w,  window_h);
    }

    if(Canvas_Storage.size() > Layer_Buttons.Menu_State() && Layer_Buttons.Menu_State() != -1 )
    {
        Canvas_Storage[Layer_Buttons.Menu_State()]->draw(20,window_h/3,color);
    }
